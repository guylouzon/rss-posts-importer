 ...
 */
    private function process_feeds(array $feeds): array {
        // The nonce is verified by process() before this method is called;
        // verify it again here so the check is in scope for every $_POST read below.
        $nonce = isset( $_POST['interq_rss_pi_nonce_field'] ) ? sanitize_key( wp_unslash( $_POST['interq_rss_pi_nonce_field'] ) ) : '';
        if ( empty( $nonce ) || ! wp_verify_nonce( $nonce, 'interq_rss_pi_save_settings_action' ) ) {
            return $feeds;
        }
